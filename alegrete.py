...
def compute_mse(theta_0, theta_1, data):
	...
	"""
	line = lambda x: theta_0 + x*theta_1
	return np.average([np.square(line(x) - y) for (x,y) in data])
...
def step_gradient(theta_0, theta_1, data, alpha):
	...
	ys = data[:,1]
	ht = line(xs)
	d0 = 2*np.average(ht - ys)
	d1 = 2*np.average((ht-ys)*xs)
	return (theta_0-alpha*d0, theta_1-alpha*d1)
